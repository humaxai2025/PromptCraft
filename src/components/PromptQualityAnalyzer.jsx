...
function PromptCraft() {
  ...
  // Check word limits (focused on words, not characters)
  const checkLimits = () => {
    ...
    return {
      ...
      chars: { count: charCount, max: currentModel.maxChars, status: 'good' },
...
Generated by Prompt Craft - https://promptcraft.app`;
  ...
  // Get word/character recommendations based on selected AI model
  const getRecommendations = () => {
    ...
    // Word recommendations based on selected model
    const optimalWords = Math.floor(currentModel.maxWords * 0.7); // 70% of max
    const wordStatus = words >= currentModel.maxWords ? 'exceeded' : 
    ...
    return {
      words: { 
        count: words, 
        status: wordStatus, 
        recommendation: `${optimalWords}-${currentModel.maxWords} words for ${currentModel.name}` 
      },
      chars: { 
        count: chars, 
        status: charStatus, 
  ...
  // Show visual feedback when AI model changes
  useEffect(() => {
    if (prompt.trim()) {
      // Re-enforce word limits when model changes
      const currentModel = aiModels[selectedAIModel];
      const words = prompt.trim().split(/\s+/).filter(word => word.length > 0);
                      ...
                      return (
                        <div className={`mb-4 p-3 rounded-lg border ${
                          ...
                          <div className="flex items-center gap-2 text-sm font-medium">
                            ...
                            <span>
                              {isAtLimit 
                                ? `Word limit reached for ${limits.modelName}!` 
                                : `Approaching word limit for ${limits.modelName}`
                              }
                            </span>
                          </div>
