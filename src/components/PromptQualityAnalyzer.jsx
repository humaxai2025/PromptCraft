...
function PromptCraft() {
  ...
  const optimizeForModel = (originalPrompt, targetModel) => {
    ...
    prefixesToRemove.forEach(prefix => {
      const cleanPrefix = prefix.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'); // Escape regex chars
...
**Usage:** Copy Part 1 above for your first message, then use Part 2 in a follow-up message.`;
  ...
  const checkLimits = () => {
    ...
    const wordStatus = wordCount >= maxWords ? 'exceeded' : 
                      wordCount >= maxWords * 0.9 ? 'warning' : 'good';
    
    return {
      words: { count: wordCount, max: maxWords, status: wordStatus },
                  ...
                  {analysis && (
                    <div className="mt-4 space-y-3">
                      ...
                      {(() => {
                        ...
                        return (
                          <div className={`text-xs p-2 rounded-lg ${lengthStatus.bgColor.replace('text-', 'border-').replace('/20', '/30')} border`}>
